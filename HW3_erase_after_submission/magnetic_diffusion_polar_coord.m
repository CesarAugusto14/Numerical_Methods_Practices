...
for n = 1:N
    for i = 2:M-1
        ...
        % Dirichlet:
        B(n+1, 1) = BC(1); 
        % B(n+1, end) = BC(2);                  % For DC
        B(n+1, end) = BC(2)*sin(2*pi*f*t(n));   % For AC
...
%% This plot is giving dynamic moving line.
% For getting the plots attached to the report, we will need to uncomment 
% the hold on and the saveas. 
for i = 2:length(B)-1
    if mod(t(i),dt*50) == 0   % For 50 ns plots!
